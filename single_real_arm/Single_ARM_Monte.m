...
T_init_angle_list = 0*datasample(T_init_data, case_num, 'Replace', false)';  % 중복 없이 30개

T_init_posX_data = min_posX:0.001:max_posX;  % 예시 데이터
T_init_posX = datasample(T_init_posX_data, case_num, 'Replace', false)';
T_init_posY_data = min_posY:0.001:max_posY;  % 예시 데이터
T_init_posY = datasample(T_init_posY_data, case_num, 'Replace', false)';
T_init_pos_list = [T_init_posX, T_init_posY] + [0.30 0];
...
for i=1:case_num
    ...
    simResult = sim('Model2_monte.slx', 'StopTime', num2str(Sim_Time));  % 각 시뮬 200초까지 실행
